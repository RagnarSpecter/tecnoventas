<?php 
	...
	class Ventas{
											...
											<tr id='contenedor_costo_y_cantidad_".$productos[$i]["codigo_producto"]."' class='contenedor_costo_y_cantidad' style='display:none'>
												<td colspan='7'>
													<span>
														...
														<span style='margin-right:20px'><input type='text' id='costo_producto_".$productos[$i]["codigo_producto"]."'  class='centrar' size='10' onkeyup='format(this)' onchange='format(this)'></span>
		...
		// Metodo para devolver una tabla con la lista de productos por marca, 
		// exclusivo para la pagina de venta de productos.
		public function lista_productos_marca_venta( $cod_familia, $cod_categoria, $cod_marca ){

			// VARIABLES
			$lista = "";

			// CUERPO
			// Instancia a la clase Trabajo que contiene el metodo de conexion
			$this->trabajo = new Trabajo;
			...
			// Primero cuento la cantidad de productos que hay con el codigo de familia elegido
			$consulta = $con->prepare( "SELECT COUNT(*) AS cantidad FROM producto WHERE cod_familia = ? AND cod_categoria = ? AND cod_marca = ?" );
			$consulta->bindValue( 1, $cod_familia );
			$consulta->bindValue( 2, $cod_categoria );
			$consulta->bindValue( 3, $cod_marca );
			$consulta->execute();
			$temporal = $consulta->fetch( PDO::FETCH_ASSOC );
			$cantidad = $temporal['cantidad'];
			...
			if( $cantidad == 0 ){	
				...
				$consulta = $con->prepare( 	"
												...
												JOIN producto_plataforma	AS propla on p.cod_plataforma = propla.cod_plataforma
												WHERE p.cod_familia = ".$cod_familia."
												AND p.cod_categoria = ".$cod_categoria."
		...
		public function cargar_carrito( $cod_producto, $precio, $cantidad ){
			...
			return "<pre>".print_r( $_SESSION["carrito"] )."</pre>";
